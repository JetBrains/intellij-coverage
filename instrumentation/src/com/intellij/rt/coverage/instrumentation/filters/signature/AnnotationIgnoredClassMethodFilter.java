...
public class AnnotationIgnoredClassMethodFilter implements MethodSignatureFilter {
  ...
  public boolean shouldFilter(int access, String name, String desc, String signature, String[] exceptions, MethodFilteringVisitor context) {
    final List<Pattern> annotationsToIgnore = context.getProjectData().getAnnotationsToIgnore();
    if (annotationsToIgnore == null || annotationsToIgnore.isEmpty()) return false;
    for (String annotation : context.getAnnotations()) {
      final String annotationName = ClassNameUtil.convertVMNameToFQN(annotation);
      if (ClassNameUtil.matchesPatterns(annotationName, annotationsToIgnore)) return true;
    }
    return false;
  }
}
